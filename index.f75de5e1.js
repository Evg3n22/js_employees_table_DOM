...
thead.addEventListener("click", (e)=>{
    ...
    Object.keys(currentSort).forEach((key)=>{
        if (Number(key) !== theadIndex) currentSort[key] = null;
    });
    ...
    rows.sort((a, b)=>{
        ...
        if (sortDirection === "asc") {
            if (isNum(convertSalary(aVal)) && isNum(convertSalary(bVal))) return parseFloat(convertSalary(aVal)) - parseFloat(convertSalary(bVal));
            return aVal.localeCompare(bVal);
        }
        if (sortDirection === "desc") {
            if (isNum(convertSalary(aVal)) && isNum(convertSalary(bVal))) return parseFloat(convertSalary(bVal)) - parseFloat(convertSalary(aVal));
...
function convertSalary(salary) {
    if (!Number.isFinite(+salary[0])) return +salary.slice(1).replace(/,/g, "");
    return +salary;
}
// Select row
  ...
  `;
    ...
    employeesForm.addEventListener("submit", (e)=>{
        ...
        if (/^[a-zA-Z]+$/.test(formData.get("name")) === false) {
            pushNotification(10, 10, "Error", "Name must contain only letters", "error");
            employeesForm.reset();
            return;
        }
        ...
        if (Number(formData.get("age")) < 18 || Number(formData.get("age")) > 90) {
            pushNotification(10, 10, "Error", "Age must be at least 18 and salary at least 90", "error");
            employeesForm.reset();
            return;
        }
...
tbody.addEventListener("dblclick", (e)=>{
    ...
    input.addEventListener("blur", ()=>{
        ...
        if (newValue === "") targetCell.textContent = originalValue;
        else {
